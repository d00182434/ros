...
#ifdef NO_CONTINUOUS_MODE
    { MESSAGE_LEN,0,"","msglen",        Arg::Numeric,  "  \t--msglen=<num>  \tMessage data length."},
#endif
...
int main(int argc, char** argv){
    ...
    for (int i = 0; i < parse.optionsCount(); ++i){
        ...
        switch (opt.index()){
            ...
            #ifdef NO_CONTINUOUS_MODE
            case MESSAGE_LEN:
                msglen = strtol(opt.arg, nullptr, 10);
                break;
            #endif
    ...
    if (pub_sub){
        ...
        #ifdef NO_CONTINUOUS_MODE
        latencyPub.init(sub_number,n_samples, reliable, seed, hostname, export_csv, msglen);
        #else
        ...
        #ifdef NO_CONTINUOUS_MODE
	latencySub.init(echo, n_samples, reliable, seed, hostname, msglen);
        #else
