...
std::vector<uint32_t> data_size_sub (datassub, datassub + 1 );
#else
...
//zhouyoujun:
#ifdef NO_CONTINUOUS_MODE
bool LatencyTestSubscriber::init(bool echo, int nsam, bool reliable, uint32_t pid, bool hostname, uint32_t msg_len)
#else
bool LatencyTestSubscriber::init(bool echo, int nsam, bool reliable, uint32_t pid, bool hostname)
#endif
{
    #ifdef NO_CONTINUOUS_MODE
    data_size_sub[0] = msg_len -4;
    #endif
